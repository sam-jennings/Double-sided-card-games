...
This section defines the scoring instrument used in §2 so every scorecard reads consistently. It does **not** redefine the rubric — the dimensions and anchors are distilled from the binding steering files, which remain authoritative.
...
| **D2** | **Physical-handling viability** | Does it survive the double-faced-card reality — no card backs, no hidden-face memory burden? | `physical-handling.md` checklist; `design-principles.md` #2–#3 |
...
- **OUROBOROS post-mortem** — `Ouroboros/OUROBOROS_design_analysis.md`: the recorded cut reasons and binding lessons (confronted in §4).
- **Binding steering** — `.kiro/steering/` (`design-principles.md`, `physical-handling.md`, `product-vision.md`, `deck-structure.md`, `repository-structure.md`): the rubric, tier semantics, handling constraints, and OUROBOROS lessons the audit applies but does not redefine.
...
*2–4 players · ~15–25 min · tactical grid (capture engine)* — folder `Trigon/` ([rulebook](Trigon/TRIGON_rulebook.md) · [analysis](Syzygy/SYZYGY_design_analysis.md))
...
| Dimension | Score | Justification (cites a specific deck property / handling technique / coverage fact) |
|---|:---:|---|
| Deck-necessity (D1)              | 9/10 | Built on pair-uniqueness: "there is exactly one Sun/Crown in the world," so a claim about a hidden face **forges a specific, unique object** that someone may be holding, and the lie-space provably shrinks (concepts "every lie about a hidden side is eventually falsifiable"). The registry kills 6–12 *named* pairs publicly; each symbol on exactly 8 cards makes lies countable. Held from 10 because the shed-by-bluffing skeleton echoes a normal-deck bluff game (Cheat/BS) — K₉ supplies the *verifiable shrinking* lie-space rather than the whole shell. |
...
*3–5 players · ~25 min · trick-taking whodunit / deduction · full 36-card deck* — folder `Unplayed Pair/` ([rulebook](Unplayed%20Pair/UNPLAYED_PAIR_rulebook.md) · no design analysis yet)
...
*3–6 players · ~25 min · simultaneous negotiation / minority game · full 36-card deck* — folder `Council/` ([rulebook](Council/COUNCIL_rulebook.md) · no design analysis yet)
...
This is the load-bearing **handling** failure, and the one cut reason that is in principle *fixable*. In OUROBOROS the playable symbol at each end of the serpent — the "open symbol" you must match to place your next card — is the **hidden face of the last card** at that end (analysis playtest verdict: "the head's open symbol is the hidden face of the last card, so the player must repeatedly check under the end of the serpent to know what's playable"). That is a textbook violation of the binding constraint in [`physical-handling.md`](.kiro/steering/physical-handling.md): *"Constantly needed state on the hidden/down face… This sank OUROBOROS: the 'open symbol' lived under the serpent's head."* The same note records the binding lesson for the whole collection (`design-principles.md` lessons 1–2): constantly-needed state must be **visible**, never the down-face.
...
Any proposed revival of an OUROBOROS mechanic must pass the binding gate from `physical-handling.md` / `design-principles.md`: it must use **visible state**, with **no hidden-face memory** and **no constantly-needed state on a down-face**. Applying the gate:
